 ...
 *   1. Collects the user-facing diff (lockfiles/tests/internal-only paths excluded).
 *   2. Finds the docs pages in the docs repo most likely affected by that diff.
 *   3. Builds one prompt (diff + current page content + a strict output contract)
 *      and hands it to a pluggable LLM CLI backend (claude | codex | gemini).
 *   4. Parses the backend's output for ===FILE:...===...===END=== blocks,
 *      writes those files into a local checkout of the docs repo, and opens
 *      a (draft, by default) PR against it via `gh`.
 *
 ...
 *   - Backend is pluggable via DOCS_AGENT_BACKEND (claude | codex | gemini)
...
const PRODUCTS = ["layer", "overwatch", "locus", "routeshift", "codex"];
...
const BACKENDS = {
  ...
  gemini: {
    ...
    // (e.g. "-p" and this script will append the prompt as a trailing arg
    // instead of piping it, see runBackend()).
    args: (process.env.DOCS_AGENT_GEMINI_ARGS || "-p -").split(" ").filter(Boolean),
    stdin: true,
  },
};

const DEFAULT_TIMEOUT_MS = Number(process.env.DOCS_AGENT_TIMEOUT_MS || 10 * 60 * 1000); // 10 min
...
function parseArgs(argv) {
  const out = {
    ...
    docsRepoPath: process.env.DOCS_AGENT_DOCS_PATH || "./axiom-docs",
    product: null,
...
Required:
  ...
  --product <layer|overwatch|locus|routeshift|codex>

Optional:
  --docs-repo-path <path>       Local checkout of the docs repo (default ./axiom-docs,
                                 or env DOCS_AGENT_DOCS_PATH). Must already exist, be a
                                 git repo, have a remote, and be gh-authenticated.
  --backend <claude|codex|gemini>  Default: env DOCS_AGENT_BACKEND, else "claude".
  ...
  --timeout-ms <n>              Backend invocation timeout (default 600000 = 10 min).
  --base-branch <name>          Docs repo base branch (default main).
...
function collectRawDiff(opts) {
  if (opts.pr != null) {
    ...
    const diff = run("gh", ["pr", "diff", String(opts.pr), "--repo", opts.repo]);
    return { rawDiff: diff.stdout, prMeta };
  }
  if (opts.range) {
...
function buildPrompt({ prMeta, filteredDiff, droppedPaths, candidatePages, docsRepoPath }) {
  ...
  return `You are drafting a documentation update for the Axiom docs site (Mintlify-flavored
MDX, being ported to Fumadocs — treat MDX syntax as portable prose + the small
...
${pagesSection || "(no candidate pages found under this product's docs directory)"}
...
- Only emit a FILE block for a page you are actually changing. If nothing in
  the candidate pages needs updating, emit NO FILE blocks and instead write a
  one-paragraph plain-text explanation of why no doc change is needed.
...
- Do not invent new pages outside the candidate list unless the diff clearly
  ...
  justify it in one sentence before the FILE block.`;
}
...
function runBackend(backendName, prompt, timeoutMs) {
  ...
  return new Promise((resolve) => {
    const child = spawn(backend.cmd, backend.args, { stdio: ["pipe", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
    let timedOut = false;
...
function parseFileBlocks(output) {
  ...
  while ((m = FILE_BLOCK_RE.exec(output)) !== null) {
    blocks.push({ relPath: m[1].trim(), content: m[2] });
  }
  return blocks;
}
...
function applyChangesAndOpenPR(opts, { fileBlocks, prMeta, backendName, droppedPaths, keptPaths, rawOutput }) {
  const changed = [];
  const unchanged = [];
  ...
  run("git", ["-C", opts.docsRepoPath, "fetch", "origin", opts.baseBranch]);
  ...
  run("git", ["-C", opts.docsRepoPath, "checkout", "-B", branchName, `origin/${opts.baseBranch}`]);
  ...
  run("git", ["-C", opts.docsRepoPath, "add", ...changed.map((c) => c.relPath)]);
...
async function main() {
  ...
  const { rawDiff, prMeta } = collectRawDiff(opts);
  ...
  const prompt = buildPrompt({ prMeta, filteredDiff, droppedPaths, candidatePages, docsRepoPath: opts.docsRepoPath });
  ...
  if (fileBlocks.length === 0 && !/no.{0,20}(doc|update|change)/i.test(rawOutput)) {
    // Zero file blocks AND the output doesn't even read like an explicit
    // "no change needed" explanation — likely the model didn't follow the
    // output contract at all. Fail loudly rather than silently treating
    // malformed output as "nothing to do".
    fail(
      `backend "${opts.backend}" produced zero parseable ===FILE:===...===END=== blocks, and its ` +
      `output doesn't read as an explicit "no doc change needed" explanation either — this looks ` +
      `like the model didn't follow the output contract, not a real no-op. Full output saved to ` +
      `${path.join(logDir, `${runId}-output.txt`)} for inspection.`
    );
  }
